...
class Video:
    ...
    def LoadBackgroundVideos(self):
        scroll = self.parent.backgrounds_frame  # QScrollArea
        scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)

        if not scroll.widget():
            content = QWidget()
        ...
        def resize_thumbnails():
            ...
            # käytettävissä oleva leveys ilman marginaaleja ja spacingia
            available_width = total_width - margins.left() - margins.right() - (cols - 1) * spacing
            card_width = math.floor(available_width / cols)  # floor estää vaakaskrollin
